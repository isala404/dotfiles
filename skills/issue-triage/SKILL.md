---
name: issue-triage
description: Use when explicitly asked to open, file, or raise GitHub issues - one, or a batch from a list of findings or TODOs. Never for a problem the user merely mentioned; filing must be asked for.
---

# Issue triage

**Requires:** `gh`, authenticated, and a git repo with a GitHub remote. If `gh auth status` fails, stop and tell the user to run `gh auth login` rather than trying another route to the API. `gh` cannot upload images — see step 4.
...
**Titles.** Name the end state, in imperative mood, with the subsystem visible:

| Bad | Why | Good |
|---|---|---|
| `Bug in upload` | Names nothing. Unscannable in a list. | `Retry transient upload failures in the worker` |
| `Fix the critical session vulnerability that allows revoked users to remain authenticated` | Inflated, and it's a paragraph. | `Reject refresh tokens for revoked sessions` |
| `Improve error handling` | Not an end state, could mean anything. | `Return 404 when a project does not exist` |
| `worker/upload.py refactor` | A file path is not an outcome. | `Split upload retry logic out of handle_upload` |

**Problem sections.** Same fact, model prose vs. plain:

> ❌ This issue aims to address a critical robustness gap in the upload subsystem. The current implementation lacks a comprehensive retry mechanism, potentially leading to suboptimal outcomes for end users and undermining the reliability of the overall data pipeline — highlighting the importance of resilient error handling.

Every clause there is padding, "critical" is invented, and it never says what actually happens.
...
The test: could someone reproduce or locate the problem from what you wrote? If not, it's decoration.
