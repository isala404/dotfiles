...
Commit messages: first check what convention the repo already uses (skim `git log`) and match it — style, casing, scopes, prefixes like Conventional Commits. If the repo has no discernible convention, default to imperative mood ("add", "fix", "remove", not "added"/"fixes"), single-line, lowercase, scoped if the project uses scopes. No AI attribution, no co-author tags, no "generated by" footers. Add a body only if the change actually needs one.
